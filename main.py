import os                      # Importamos funciones del sistema operativo, como la manipulación de rutas.
import random                  # Importamos para generar colores aleatorios para los objetos.
...
# Definimos el nombre del video base (sin extensión)
name_video = 'cafeteria'
...
# Cargamos el modelo YOLOv8 versión "nano", que es ligero y rápido
...
while ret:
    ...
    for result in results:
        ...
        for track in tracker.tracks:
            bbox = track.bbox                          # Obtenemos la caja delimitadora del objeto
            x1, y1, x2, y2 = bbox
            track_id = track.track_id                  # Obtenemos el ID único asignado al objeto

            # Dibujamos la caja alrededor del objeto con el color correspondiente a su ID
            cv2.rectangle(
                frame,
                (int(x1), int(y1)),
                (int(x2), int(y2)),
                (colors[track_id % len(colors)]),       # Seleccionamos el color según el ID
